...
public class BasicDAO<T> { //泛型指定具体类型
    ...
    //开发通用的dml方法，针对任意的表
    public int update(String sql,Object... parameters){ //可变形参
    ...
    /**
     * 
     * @param sql sql语句，可以有?问号
     * @param clazz 传入一个类的Class对象，eg.Actor.class
     * @param parameters 传入?的具体的值，可以是多个
     * @return 根据Actor.class 返回对应的ArrayList集合
     */
    public List<T> queryMulti(String sql,Class<T> clazz,Object... parameters){ //返回集合的类型不确定，所以用泛型
        Connection connection = null;
        try {
            connection = JDBCUtilsByDruid.getConnection();
